...
export default function QRPanel() {
  ...
  const handleFunctionalizeDone = useCallback(async (updatedApp: SleekPreviewApp) => {
    ...
    try {
      ...
      const appJs = buildSnackApp(updatedApp.appName, updatedApp.screens.map((s) => s.name));
      // Call exp.host directly from the browser — avoids Vercel 10s serverless timeout
      const res = await fetch("https://exp.host/--/api/v2/snack", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: updatedApp.appName,
          description: "Generated by Evermade",
          sdkVersion: "52.0.0",
          files: { "App.js": { type: "CODE", contents: appJs } },
        }),
      });
      if (res.ok) {
        ...
        console.error("[snack] error:", res.status, await res.text().catch(() => ""));
      }
    } catch (err) {
